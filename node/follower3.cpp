...
{ROS_INFO("follow depth [%d]", countkk++);
...
int main(int argc, char **argv)
{
  ...
  MyNodeHandle node;
  ros::Subscriber depthRawSub = node.subscribe("/camera/depth/image_raw", 10, depthCallback);
  ros::Subscriber rgbRawSub = node.subscribe("/camera/rgb/image_raw", 10, rgbCallback);
